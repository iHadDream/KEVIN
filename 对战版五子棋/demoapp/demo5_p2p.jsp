...
<%
	...
	if(ip==null||"".equals(ip.trim())){
		...
		if("?".equals(info)||"？".equals(info)){
			...
			Object tmpTime = request.getParameter("time");
			Long time ;
			if(tmpTime==null){
				time = null;
			}else{
				time= Long.valueOf(tmpTime.toString());
			}
			...
			if(null==ipmsg){
				...
				if(time!=null){
					...
					cur.setTime(cur.getTime()-3*60*1000);
				}
				for(IPMSG m : ipmsg){
					//if(targetip.equals(m.toip)){
					if(cur.getTime() < m.createTime.getTime()){	
						//tmp.add(m);
						if("public".equals(m.toip)||targetip.equals(m.toip)){
							rst +=  "<msg>";
	...
	rst = "<time>" + new java.util.Date().getTime() + "</time>" + rst;
